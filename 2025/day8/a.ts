...
//WARN: 10 for test case, update to 1000 with real input
const topK = edges.slice(0, 1000);
...
for (const edge of topK) {
  dsu.union(edge.idOne, edge.idTwo);
}

// getting top three
const topThree = dsu.sizes.sort((a, b) => b - a).slice(0, 3);

// logging the value
console.log(topThree.reduce((acc, val) => acc * val));
